...
// originRefererFor 出站 Origin/Referer 基址：国内版 codebuddy.cn，国际版 codebuddy.ai。
...
// userAgentFor 返回该账号的出站 UA：Client.UserAgent 非空则覆盖（全部出站请求生效），
// 否则按区域给 —— 国内 CLI/CodeBuddy 品牌，国际 WorkBuddy 品牌（见 ua.go）。
func (c *Client) userAgentFor(a *auth.Auth) string {
	if c != nil && c.UserAgent != "" {
		return c.UserAgent
	}
	return userAgentFor(a)
}
