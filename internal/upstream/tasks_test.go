...
func TestClaimRewardWebEndpoint(t *testing.T) {
	...
	// 成长中心域必须走 webBase 收口：国内 workbuddy.cn / 国际 workbuddy.ai
	//（测试把 WebBaseCN 注入成 srv.URL，所以这里断言"等于 webBase"而不是某个写死的域名）。
	if want := c.webBase(a) + "/profile/growth-center"; gotReferer != want {
		t.Errorf("Referer=%q want %q（须走 webBase 收口，不能写死域名）", gotReferer, want)
	}
}
