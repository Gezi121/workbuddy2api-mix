...
// 上游用它表达"该模型的使用量超限"（code 6004，msg 带「将在 … 重置」或英文「reset at …」），
// 而不是账号整体被限流——账号健康，只是这个模型此刻被限（issue #31）。
const modelRateLimitCode = "6004"

// softRateResetPattern 匹配中文「将在 … 重置」与英文「reset at …」，捕获时间串。
...
// ParseSoftRateReset 从 429 body 解析重置时间（支持中英文文案，时区固定 UTC+8）。
// 成功返回解析出的**墙钟时刻**（按 UTC+8 解释），失败返回零值 + false。
// 内部先判 IsModelRateLimit：非模型级限流（非 6004）即使带"重置"字样也不返回。
...
// chatBase 返回该账号的对话域：国内版 copilot.tencent.com，国际版 codebuddy.ai。
//
// 区域由账号凭证里的 domain 决定（见 region.go）—— 国际版账号落盘时 domain 是
// *.ai，于是这里自动走国际版域，调用方不必感知区域。
...
// billingBase 返回该账号的 billing 域：国内版 codebuddy.cn，国际版 codebuddy.ai。
...
// webBase 返回该账号的官网域（任务领奖 / 成长中心类接口）：国内版 workbuddy.cn，
// 国际版 workbuddy.ai。`WebBaseCN` 未注入时回落国内默认。
...
// modelsPathFor 返回该账号的模型目录路径。
//
// 两版**不同**（实测）：
//   - 国内版在 CLI 域上用 `/console/enterprises/personal/models`（国内项目一直这么用）；
//   - 国际版（workbuddy.ai）该路径是 **Web 路由** —— 不带 cookie、只带 Bearer 打过去
//     会被网关返回 500（nginx 的 HTML 错误页），也就是"拉取模型列表失败：
//     models api status 500"。国际版可用的是 `/v2/enterprises/personal/models`
//     （与专职国际版的项目一致，实测该路径返回 401 需要鉴权 = 真实 API）。
func modelsPathFor(a *auth.Auth) string {
	if RegionOf(a) == RegionINTL {
...
// supplementalModels 返回上游模型目录**漏发**、但实测可直接调用的模型（按区域）。
//
// 国际版实测（2026-09-13，真实国际账号）：`/v2/enterprises/personal/models` 的
// agents[cli].models 里**没有** `deepseek-v4.1-flash`，但拿它对话返回 HTTP 200
// （同一时刻 `deepseek-v4-flash` / `deepseek-v4-pro` 返回 11102
// "model service info not found"）—— 所以这是目录漏发，不是模型不可用。
//
// 补进列表不影响对话本身：请求体的 model 是**透传**的、不经列表校验
// （见 payload.go 的"未知模型一律透传"）。补它只是让 `/v1/models` 与面板的
// 模型选择器能看见这个模型。
//
// 元数据取国内版目录里同名模型的定义（国际版目录没给这个模型的字段）。
func supplementalModels(a *auth.Auth) []ModelInfo {
	if RegionOf(a) != RegionINTL {
		return nil
	}
	return []ModelInfo{{
...
func (c *Client) FetchModels(a *auth.Auth) ([]ModelInfo, error) {
	url := c.chatBase(a) + modelsPathFor(a)
	...
	// 上游目录漏发的模型补上（见 supplementalModels）：对话是透传的、不受影响，
	// 补上只为让 /v1/models 与面板选择器看得见。
	for _, sm := range supplementalModels(a) {
		dup := false
		for _, mi := range out {
			if mi.ID == sm.ID {
				dup = true
