...
func (c *Client) ClaimReward(a *auth.Auth, taskCode string) (credit, energy int64, err error) {
	...
	// Web 端请求头形状（对照浏览器实际请求）：Origin/Referer 指向**该账号所在区域**的
	// 成长中心（国内 workbuddy.cn / 国际 workbuddy.ai），带 x-client-platform: web 标记来源端。
	req.Header.Set("Authorization", "Bearer "+a.AccessToken)
	req.Header.Set("Accept", "application/json, text/plain, */*")
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Origin", c.webBase(a))
	req.Header.Set("Referer", c.webBase(a)+"/profile/growth-center")
	req.Header.Set("x-client-platform", "web")
	if ua := c.userAgentFor(a); ua != "" {
