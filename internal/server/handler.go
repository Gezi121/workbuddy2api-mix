...
func NewHandler(cfg Config) *Handler {
	...
	// 兜底：未匹配的请求先记一行日志再 404。
	//
	// 诊断用：OpenAI 兼容客户端除了 /v1/chat/completions，有的还会探测别的路径
	// （/v1/models/{id}、/v1/completions，或者 base_url 末尾的 /v1 被拼重成
	// /v1/v1/...）。这些以前静默 404、日志里一片空白，客户端只报一句
	// "status=404 Not Found"，看不出它到底请求了什么。记一行就能对上。
	h.mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		log.Printf("unmatched %s %s ua=%q auth=%v len=%d",
			r.Method, r.URL.Path, r.UserAgent(), r.Header.Get("Authorization") != "", r.ContentLength)
		http.NotFound(w, r)
	})
	return h
}
		...
		if status >= 400 {
			...
			if kind == upstream.ErrSoftRate && upstream.IsModelRateLimit(string(respBody)) {
				log.Printf("WARN: [server] chat uid=%s: model %s hit 6004 rate limit, rotating to next account (attempt %d/%d)",
					acct.UID, peek.Model, i+1, h.cfg.MaxRotate)
			}
