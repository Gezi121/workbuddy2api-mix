...
// 区别于 Cooldown：当上游 6004 明说重置时间时，把冷却截止精确设为
// resetAt（上游给定时间，不再靠固定基数+指数退避猜测），并记录触发模型到独立的 modelCooldowns，
// 仅该模型在该账号上处于冷却，该账号对其他模型完全可用；且该单模型限流不计入熔断器（避免账号被误锁）。
func (p *Pool) CooldownSoftForModel(uid string, base time.Duration, resetAt time.Time, model, reason string) {
	...
	if e, ok := p.byUID[uid]; ok {
		...
		if hasReset {
			...
			// 6004 模型级限流属于正常频率限制，绝不计入熔断器（避免账号被误锁死）
		} else {
			e.softRateModel = ""
			p.recordBreakerFailureLocked(e) // 无时间文案的限流才喂熔断
		}
		p.dirty.Store(true)
	}
}
