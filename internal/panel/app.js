...
function openAdd() {
  ...
  stopPoll();
  // 区域随弹窗里的下拉走：国内版打 copilot.tencent.com / codebuddy.cn，
  // 国际版打 codebuddy.ai —— 拿回来的凭证 domain 决定后续业务请求走哪套域。
  const region = ($('addRegion') && $('addRegion').value) || 'cn';
  api('login/start?region=' + encodeURIComponent(region), { method: 'POST' }).then(r => {
    loginState = r.state;
...
// 切换区域 = 重新发起一次授权（旧的 state 作废，轮询停掉）
$('addRegion').onchange = () => {
  if ($('addVeil').classList.contains('on')) openAdd();
};
